...
parser.add_argument('--filename', help='file to store output', default='pose_output.txt')
...
def main():
    ...
    def render_overlay(engine, output, src_size, inference_box):
        ...
        for pose in outputs:
            ...
            for label, keypoint in pose.keypoints.items():
                with open(filename, 'a') as outfile:
                    sv_pose_str = '  %-20s x=%-4d y=%-4d score=%.1f' % (label.name, keypoint.point[0], keypoint.point[1], keypoint.score)
